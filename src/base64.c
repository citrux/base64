...
const char alphabet[64] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
int encode(const char *input, uint32_t input_length, char *output) {
    ...
    while(read_position < input_length) {
        ...
        output[write_position++] = get_alphabet_symbol(buffer>>2);
        buffer <<= 6;
        buffer |= (chunk & ((1 << (8-bits_in_buffer)) - 1)) << (6 - bits_in_buffer);
        bits_in_buffer += 2;
        if (bits_in_buffer == 6) {
            output[write_position++] = get_alphabet_symbol(buffer>>2);
            buffer = 0;
...
int decode(const char *input, uint32_t input_length, char *output) {
    ...
    while(read_position < input_length) {
        ...
        output[write_position] = get_alphabet_index(c) << 2;

        read_position += 1;
        c = input[read_position];
        output[write_position] |= get_alphabet_index(c) >> 4;
        write_position += 1;
        output[write_position] = get_alphabet_index(c) << 4;
        ...
        output[write_position] |= get_alphabet_index(c) >> 2;
        write_position += 1;
        output[write_position] = get_alphabet_index(c) << 6;

        read_position += 1;
        c = input[read_position];
        if (c == '=') { return 0; }
        output[write_position++] |= get_alphabet_index(c);
        read_position += 1;
    }
    return 0;
}
